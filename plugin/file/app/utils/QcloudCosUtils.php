...
class QcloudCosUtils
{
    ...
    private static function initOssClient()
    {
        ...
        return self::$cosClient = new Client([
            'region'      => config('plugin.file.superadminx.file_system.qcloud.region'),
            'scheme'      => 'https', //协议头部，默认为 http
            'credentials' => [
                'secretId'  => config('plugin.file.superadminx.file_system.qcloud.SecretId'),
                'secretKey' => config('plugin.file.superadminx.file_system.qcloud.SecretKey')
            ]
        ]);
    }
    ...
    public static function upload(string $filePath, bool $deleteFile = true) : string
    {
        try {
            ...
            $result = (self::initOssClient())->Upload(
                config('plugin.file.superadminx.file_system.qcloud.bucket'),
                $ossPath,
                fopen(public_path() . $filePath, 'rb')
            );

            // 上传后访问的连接
            $url = config('plugin.file.superadminx.file_system.qcloud.bucket_url') . "/{$ossPath}";
            //存入file表数据库
    ...
    public static function delete(string $object) : void
    {
        ...
        try {
            $params = [
                'Bucket' => config('plugin.file.superadminx.file_system.qcloud.bucket'),
                'Key'    => $object,
            ];
            if ($version_id) {
    ...
    public static function getSignature(string $dir)
    {
        ...
        $bucket     = config('plugin.file.superadminx.file_system.qcloud.bucket');
        $region     = config('plugin.file.superadminx.file_system.qcloud.region');
        $bucket_url = config('plugin.file.superadminx.file_system.qcloud.bucket_url');
    ...
    public static function uploadQcloudOssCallback() : void
    {
        ...
        try {
            // 获取文件的元信息
            $exist = self::initOssClient()->getObjectMeta(config('plugin.file.superadminx.file_system.qcloud.bucket'), $post['filename']);
            // 获取文件的版本id
            $version_id = $exist['x-oss-version-id'] ?? null;
            if (config('plugin.file.superadminx.clear_file')) {
                FileLogic::create(
                    'qcloud',
                    config('plugin.file.superadminx.file_system.qcloud.bucket_url') . "/{$post['filename']}",
                    $post['size'],
                    $post['filename'],
                    $version_id
                );
