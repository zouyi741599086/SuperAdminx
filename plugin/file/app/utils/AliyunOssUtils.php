...
class AliyunOssUtils
{
    ...
    private static function initOssClient()
    {
        ...
        return self::$ossClient = new OssClient(
            config('plugin.file.superadminx.file_system.aliyun.AccessKeyID'),
            config('plugin.file.superadminx.file_system.aliyun.AccessKeySecret'),
            config('plugin.file.superadminx.file_system.aliyun.endpoint')
        );
    }
    ...
    public static function upload(string $filePath, bool $deleteFile = true) : string
    {
        try {
            ...
            $result = self::initOssClient()->uploadFile(
                config('plugin.file.superadminx.file_system.aliyun.bucket'),
                $ossPath,
                "./public/{$filePath}"
            );
            // 上传后访问的连接
            $url = config('plugin.file.superadminx.file_system.aliyun.bucket_url') . "/{$ossPath}";
            //存入file表数据库
    ...
    public static function signUrl(string $object, int $timeout = 0) : string
    {
        ...
        try {
            ...
            // 生成签名URL。
            return self::initOssClient()->signUrl(config('plugin.file.superadminx.file_system.aliyun.bucket'), $object, $timeout, "GET", $options);
        } catch (OssException $e) {
            abort($e->getMessage());
        }
    }
...
// 		$conditions[] = $start;
        ...
        $signature      = base64_encode(hash_hmac('sha1', $string_to_sign, config('plugin.file.superadminx.file_system.aliyun.AccessKeySecret'), true));

        $response['accessid']  = config('plugin.file.superadminx.file_system.aliyun.AccessKeyID');
        $response['host']      = config('plugin.file.superadminx.file_system.aliyun.bucket_url');
    ...
    public static function uploadAliyunOssCallback() : void
    {
        ...
        try {
            // 获取文件的元信息
            $exist = self::initOssClient()->getObjectMeta(config('plugin.file.superadminx.file_system.aliyun.bucket'), $post['filename']);
            // 获取文件的版本id
            $version_id = $exist['x-oss-version-id'] ?? null;
            if (config('plugin.file.superadminx.clear_file')) {
                FileLogic::create(
                    'aliyun',
                    config('plugin.file.superadminx.file_system.aliyun.bucket_url') . "/{$post['filename']}",
                    $post['size'],
                    $post['filename'],
                    $version_id
                );
