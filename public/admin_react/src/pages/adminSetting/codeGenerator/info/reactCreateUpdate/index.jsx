...
export default ({ tableName, operationFile, ...props }) => {
    ...
    useEffect(() => {
        if (data && tableColumns.length > 0) {
            ...
            // 把数据库有的 按照顺序压进去
            for (let key in data?.react_create_update?.form_fields_type) {
            ...
            tableColumns.map(item => {
                if (!data?.react_create_update?.form_fields_type?.[item.Field]) {
                    newTableColumns.push(item);
                }
            })
    ...
    const prefix = (Field) => {
        return <ProFormText
            key="prefix"
            name={['react_create_update', 'form_fields_type_config', `${Field}`, `prefix`]}
            placeholder="请输入..."
            fieldProps={{
                prefix: '前缀',
    ...
    const suffix = (Field) => {
        return <ProFormText
            key="suffix"
            name={['react_create_update', 'form_fields_type_config', `${Field}`, `suffix`]}
            placeholder="请输入..."
            fieldProps={{
                prefix: '后缀',
                style: { margin: '2px 0px' },
    ...
    const fileSuffix = (Field) => {
        return <ProFormText
            ...
            fieldProps={{
                prefix: '允许文件后缀',
                style: { margin: '2px 0px' },
                suffix: <Tooltip title="用英文逗号隔开如：ppt,docx">
                    <QuestionCircleOutlined />
                </Tooltip>
            }}
            rules={[
        ...
        <ProForm
            ...
            onFinish={async (values) => {
                adminCodeGeneratorApi.generatorCode({
                    ...
                    react_form_code: values.react_form_code,
                    table_name: tableName,
            ...
            >
                ...
                <DragSortTable
                    ...
                    columns={[
                        ...
                        {
                            title: '表单字段类型',
                            dataIndex: 'form_fields_type',
                            render: (_, record) => <>
                                <ProFormSelect
                                    name={['react_create_update', 'form_fields_type', `${record.Field}`]}
                                    placeholder="请选择"
                                    request={async () => formFieldsTypes}
                                    fieldProps={{
                                        showSearch: true,
                                        popupMatchSelectWidth: false,
                                    }}
                        ...
                        },
                        {
                            ...
                            width: 350,
                            render: (_, record) => <>
                                <ProFormDependency name={[['react_create_update', 'form_fields_type', record.Field]]}>
                                    {({ react_create_update }) => {
                                        if (react_create_update?.form_fields_type?.[record.Field]) {
                                            const formType = react_create_update?.form_fields_type?.[record.Field];
                                            ...
                                            if (formType == 'Text' || formType == 'Digit') {
                                                _component.push(prefix(record.Field))
                                                _component.push(suffix(record.Field))
                                            }
