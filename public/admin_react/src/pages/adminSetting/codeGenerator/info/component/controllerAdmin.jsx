        ...
        <ProForm
            ...
            onFinish={async (values) => {
                adminCodeGeneratorApi.generatorCode({
                    controller_admin: {
                        ...values.controller_admin, //只要form中的这些值
                        file_suffix: 'php', //生成文件的后缀名称
                    },
                    table_name: tableName,
                    code_name: 'controller_admin', //生成的代码名称
                }).then(res => {
                    if (res.code === 1) {
                        message.success(res.message);
                        formRef.current.setFieldValue('controller_admin_code', res.data.controller_admin_code);
                    } else {
            ...
            <Space direction="vertical" style={{ width: '100%' }}>
                <Row gutter={[24, 0]}>
                    <Col xs={24} sm={12} md={12} lg={8} xl={6} xxl={6}>
                        <ProFormText
                            name={['controller_admin', 'file_name']}
                            label="控制器类名"
                            placeholder="请输入"
                            rules={[
                    ...
                    <Col xs={24} sm={12} md={12} lg={8} xl={6} xxl={6}>
                        <ProFormText
                            name={['controller_admin', 'file_path']}
                            label="命名空间"
                            placeholder="请输入"
                    ...
                    <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                        <ProFormCheckbox.Group
                            name={['controller_admin', 'functions']}
                            label="生成的方法"
                            options={[
                                {
                                    label: '获取列表',
                    ...
                    <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                        <ProFormCheckbox.Group
                            name={['controller_admin', 'functions_auth']}
                            label="需要验证接口级别权限的方法"
                            options={[
                                {
                                    label: '获取列表',
                ...
                <ProFormList
                    name={['controller_admin', 'other_functions']}
                    label="添加其它方法"
                    creatorButtonProps={{
                ...
                <ProForm.Item name="controller_admin_code" >
            ...
            <ProCard boxShadow>
                <Flex align="center" justify="center" gap="small">
                    ...
                    <Button
                        danger
                        onClick={() => {
                            operationFile('controller_admin');
                        }}
                    >生成到项目</Button>
                </Flex>
            </ProCard>
