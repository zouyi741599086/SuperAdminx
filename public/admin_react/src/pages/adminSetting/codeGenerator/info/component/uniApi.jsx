        ...
        <ProForm
            ...
            onFinish={async (values) => {
                adminCodeGeneratorApi.generatorCode({
                    uni_api: {
                        ...values.uni_api, // 只要form中的这些值
                        file_suffix: 'js', // 生成文件的后缀名称
                    },
                    table_name: tableName,
                    code_name: 'uni_api', // 生成的代码名称
                }).then(res => {
                    if (res.code === 1) {
                        message.success(res.message);
                        formRef.current.setFieldValue('uni_api_code', res.data.uni_api_code);
                    } else {
        ...
        >
            <Space direction="vertical" style={{ width: '100%' }}>
                <Row gutter={[24, 0]}>
                    ...
                    <Col xs={24} sm={12} md={12} lg={8} xl={6} xxl={6}>
                        <ProFormRadio.Group
                            name={['uni_api', 'generator_type']}
                            label="api生成来源"
                            rules={[
                                { required: true, message: '请输入' }
                            ]}
                            options={uni_api_types}
                        />
                    </Col>
                </Row>

                <ProForm.Item name="uni_api_code" >
                    <CodeHighlight/>
                </ProForm.Item>
            </Space>
